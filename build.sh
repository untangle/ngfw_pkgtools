...
# Git 2.39+ (Bookworm) rejects operations on directories owned by
# other users; in CI the source is mounted from the host into a
# container running as root, so we must trust all paths.  This is
...
is-official-branch() {
  echo $1 | grep -qP '^(main|master|ngfw-bookworm|((ngfw|waf)-)?release-[\d.]+)$'
}
