...
create-existence-chroot:
	if [ ! -d $(CHROOT_EXISTENCE) ] ; then \
          ...
          sudo cp -f $(CHROOT_UPDATE_EXISTENCE_SCRIPT) $(CHROOT_EXISTENCE) ; \
          sudo cowbuilder --execute --save-after-exec --basepath $(CHROOT_EXISTENCE) -- /$(shell basename $(CHROOT_UPDATE_EXISTENCE_SCRIPT)) $(REPOSITORY) $(DISTRIBUTION) 2> /dev/null ; \
