...
move-debian-files:
	@find .. -maxdepth 1 -name "*`perl -pe 's/^.+://' $(VERSION_FILE)`*" -regex '.*\.\(upload\|changes\|udeb\|deb\|upload\|dsc\|build\|diff\.gz\|debian\.tar\.xz\)' -exec mv "{}" `cat $(DESTDIR_FILE)` \;
...
source: checkroot parse-changelog
	quilt pop -a || true
	tar cz --exclude="*stamp*.txt" --exclude="*stamp" \
